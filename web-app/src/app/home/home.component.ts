...
type EmployeeRole = 'associate' | 'sm' | 'dm' | 'ap';
...
export class HomeComponent {
  ...
  sscCaseNumber = '';
  incidentGuide: { title: string; steps: string[]; videoTitle: string } | null = null;
  ...
  employeeScanMessage = '';
  arrivalAlertsEnabled = localStorage.getItem('dt2031-arrival-alerts') === 'true';
  arrivalAlertMessage = '';
  backroomSearch = '';
  teamMessages = [
    {
  ...
  readonly socialFinds = [
    { art: '🍹', title: 'Colorful summer sippers', user: '@valuefindsva', likes: 284, tone: 'neon' },
    { art: '🧴', title: 'My skin has been loving this find', user: '@chesapeakebeauty', likes: 196, tone: 'beauty' },
    { art: '🍿', title: 'Movie-night upgrade for less', user: '@snackhunter', likes: 417, tone: 'snack' },
    { art: '👜', title: 'Beach, please — summer tote find', user: '@coastalvalue', likes: 328, tone: 'beach' },
    { art: '🫧', title: 'A little self-care shelf discovery', user: '@bathandbargains', likes: 153, tone: 'bath' },
    { art: '🧻', title: 'Modern home essentials spotted', user: '@organizedforless', likes: 221, tone: 'home' },
    { art: '💄', title: 'The cutest beauty aisle surprise', user: '@glowonabudget', likes: 506, tone: 'lip' },
    { art: '🌴', title: 'Tropical cups made for summer', user: '@partyfinds', likes: 372, tone: 'tropical' },
  ];
  readonly shoppingMissions = [
    { name: 'Birthday Party', icon: '🎉', productIds: ['DT1010', 'DT1011', 'DT1012', 'DT1005', 'DT1009'] },
    { name: 'Teacher Supplies', icon: '🏫', productIds: ['DT1013', 'DT1014', 'DT1015', 'DT1019'] },
    { name: 'Camping', icon: '🏕️', productIds: ['DT1007', 'DT1005', 'DT1027', 'DT1012'] },
    { name: 'New Apartment', icon: '🏠', productIds: ['DT1001', 'DT1002', 'DT1019', 'DT1020'] },
    { name: 'Movie Night', icon: '🍿', productIds: ['DT1006', 'DT1004', 'DT1008', 'DT1011'] },
    { name: 'Baby Shower', icon: '🍼', productIds: ['DT1010', 'DT1011', 'DT1024', 'DT1009'] },
    { name: 'Road Trip', icon: '🚗', productIds: ['DT1005', 'DT1007', 'DT1009', 'DT1027'] },
    { name: 'Spring Cleaning', icon: '🧹', productIds: ['DT1001', 'DT1002', 'DT1003', 'DT1021'] },
  ];
  demoStep = 0;
  readonly demoSteps = [
    ...
    'Open the Employee Portal Demo.',
    'Ask Bucky Team Assist what should be stocked next.',
    'Review backroom and shelf inventory.',
    'Mark the paper towel stocking task complete.',
    'Confirm the product inventory update.',
    'Open truck tracking.',
    'Show Solink Smart Store Intelligence.',
  ];
  ...
  readonly quickActions = [
    ...
    { label: 'Arriving Today', icon: 'local_shipping', prompt: 'What is arriving today?' },
    { label: 'Cheaper Alternative', icon: 'savings', prompt: 'Suggest a cheaper alternative.' },
    { label: 'Similar Products', icon: 'swap_horiz', prompt: 'Is there another similar product available?' },
  ];

  readonly businessOutcomes = [
    { icon: 'trending_up', value: '+2.8%', label: 'Sales opportunity', detail: 'Fewer lost sales from empty shelves', tone: 'revenue' },
    { icon: 'schedule', value: '11 hrs', label: 'Labor returned weekly', detail: 'Less searching, counting, and task routing', tone: 'labor' },
    { icon: 'inventory_2', value: '97.4%', label: 'On-shelf availability', detail: 'Vision-triggered restocking closes gaps faster', tone: 'availability' },
    { icon: 'shield', value: '-14%', label: 'Shrink opportunity', detail: 'Exception-based review for authorized leaders', tone: 'shrink' },
  ];

  readonly districtStores = [
    { id: '2031', city: 'Chesapeake Future Store', score: 96, status: 'green', issue: 'Running normally' },
    { id: '1847', city: 'Virginia Beach', score: 81, status: 'yellow', issue: 'Paper goods stockout risk' },
    { id: '2214', city: 'Norfolk', score: 93, status: 'green', issue: 'Running normally' },
    { id: '1189', city: 'Suffolk', score: 68, status: 'red', issue: 'Truck delay affecting 3 aisles' },
    { id: '2470', city: 'Portsmouth', score: 87, status: 'yellow', issue: 'Checkout wait above target' },
    { id: '1902', city: 'Hampton', score: 95, status: 'green', issue: 'Running normally' },
  ];

  readonly executiveRecommendations = [
    { icon: 'water_drop', title: 'Prevent a bottled-water stockout', action: 'Move 3 cases to the floor by 3:30 PM.', value: '$420 sales protected', confidence: 94 },
    { icon: 'point_of_sale', title: 'Open Register 5 at 5:05 PM', action: 'Predicted rush begins at 5:14 PM.', value: '43% lower wait time', confidence: 92 },
    { icon: 'local_shipping', title: 'Adjust today’s receiving plan', action: 'Truck DT-4821 is projected 18 minutes late.', value: '36 labor minutes recovered', confidence: 89 },
    { icon: 'storefront', title: 'Increase cleaning allocation', action: 'Regional demand is 34% above last week.', value: '12% shipment increase', confidence: 91 },
  ];

  readonly weeklySchedule = [
    { day: 'Monday', date: 'Jul 27', time: '9:00 AM–5:00 PM', role: 'Stocking + Customer Support', status: 'Complete' },
    { day: 'Tuesday', date: 'Jul 28', time: '9:00 AM–5:00 PM', role: 'Stocking + Customer Support', status: 'Complete' },
    { day: 'Wednesday', date: 'Jul 29', time: '9:00 AM–5:00 PM', role: 'Stocking + Customer Support', status: 'Today' },
    { day: 'Thursday', date: 'Jul 30', time: '11:00 AM–7:00 PM', role: 'Sales Floor', status: 'Scheduled' },
    { day: 'Friday', date: 'Jul 31', time: 'Off', role: 'Day off', status: 'Off' },
    { day: 'Saturday', date: 'Aug 1', time: '8:00 AM–4:00 PM', role: 'Truck + Stocking', status: 'Scheduled' },
    { day: 'Sunday', date: 'Aug 2', time: 'Off', role: 'Day off', status: 'Off' },
  ];

  readonly zeroSalesByProduct: Record<string, { unitsToday: number; lastSale: string; zeroSalesHours: number; expectedDaily: number }> = {
    DT1001: { unitsToday: 0, lastSale: 'Yesterday at 6:42 PM', zeroSalesHours: 17, expectedDaily: 12 },
    DT1007: { unitsToday: 8, lastSale: 'Today at 10:18 AM', zeroSalesHours: 0, expectedDaily: 14 },
    DT1010: { unitsToday: 3, lastSale: 'Today at 9:56 AM', zeroSalesHours: 0, expectedDaily: 7 },
    DT1016: { unitsToday: 0, lastSale: '2 days ago at 4:11 PM', zeroSalesHours: 42, expectedDaily: 6 },
    DT1019: { unitsToday: 1, lastSale: 'Today at 8:37 AM', zeroSalesHours: 0, expectedDaily: 4 },
  };

  readonly categories = [
    { name: 'Halloween', icon: '🎃', tone: 'orange', prompt: "I'm decorating for Halloween." },
    { name: 'Toys', icon: '🚒', tone: 'blue', prompt: 'Show me toys.' },
    { name: 'Floral & Vases', icon: '🏺', tone: 'cream', prompt: 'Show me floral and vases.' },
    { name: 'Batteries', icon: '🔋', tone: 'green', prompt: 'Where are batteries?' },
    { name: 'Hardware', icon: '🧰', tone: 'navy', prompt: 'Show me hardware.' },
    { name: 'Dinnerware', icon: '🍽️', tone: 'gray', prompt: 'Show me dinnerware.' },
    { name: 'Health & Beauty', icon: '👓', tone: 'pink', prompt: 'Show me health and beauty.' },
    { name: 'Storage', icon: '🫙', tone: 'aqua', prompt: 'Is there more storage bins?' },
    { name: 'Food', icon: '🌭', tone: 'yellow', prompt: 'Show me food.' },
    { name: 'Snacks', icon: '🍪', tone: 'brown', prompt: 'Show me snacks.' },
    { name: 'Beverages', icon: '💧', tone: 'sky', prompt: 'Show me beverages.' },
    { name: 'Food Storage', icon: '🥡', tone: 'mint', prompt: 'Show me food storage.' },
  ];
  ...
  readonly deals = [
    ...
    { icon: '🍬', name: 'Select Candy', note: 'Movie night favorites', price: '$1' },
  ];
  ...
  readonly kpis = [
    { icon: 'local_shipping', label: 'Truck #204', value: 'Arrived 10:56 AM', tone: 'green' },
    { icon: 'shelves', label: 'Aisle 7', value: '92% stocked', tone: 'blue' },
    { icon: 'inventory_2', label: 'Backroom', value: '1,245 items waiting', tone: 'amber' },
    { icon: 'groups', label: 'Checkout wait', value: '2 minutes', tone: 'purple' },
  ];
  ...
  get selectedStore() {
    return this.data.stores.find((store) => store.storeId === this.selectedStoreId) ?? this.data.stores[0];
  }
  ...
  get cartCount(): number {
    return Object.values(this.data.cart()).reduce((sum, count) => sum + count, 0);
  }

  get cartTotal(): number {
    return Object.entries(this.data.cart()).reduce((sum, [id, quantity]) => {
      const product = this.data.products().find((item) => item.productId === id);
  ...
  get lowStockProducts(): FutureProduct[] {
    return this.data.products().filter((product) =>
      ['Low Stock', 'Out of Stock', 'In Backroom', 'Restocking Now'].includes(product.inventoryStatus),
    );
  }

  get dollarProducts(): FutureProduct[] {
    return this.data.productsForStore(this.selectedStoreId).filter((product) => product.dollarZone);
  }
  ...
  get recommendedProducts(): FutureProduct[] {
    const products = this.data.productsForStore(this.selectedStoreId).filter((product) => product.totalInventory > 0);
  ...
  get taskCompletion(): number {
    ...
    return tasks.length ? Math.round(tasks.filter((task) => task.status === 'Complete').length / tasks.length * 100) : 0;
  }
  ...
  get mapPanelProducts(): FutureProduct[] {
    ...
    if (this.selectedMapDepartment) {
      return products.filter((product) => product.category === this.selectedMapDepartment).slice(0, 9);
    }
    return products.filter((product) => product.aisle === this.highlightedAisle).slice(0, 9);
  }

  get missionProducts(): FutureProduct[] {
    return this.missionProductIds
      .map((id) => this.data.productsForStore(this.selectedStoreId).find((product) => product.productId === id))
  ...
  get backroomResults(): FutureProduct[] {
    ...
    return this.data.products()
      .filter((product) => product.backroomQuantity > 0 && (!query || `${product.name} ${product.category}`.toLowerCase().includes(query)))
  ...
  setView(view: ViewMode): void {
    ...
    if (view === 'manager' && (!this.employeeLoggedIn || this.employeeRole === 'associate')) {
  ...
  loginEmployee(): void {
    ...
    this.loginError = 'Use the fictional demo credentials: Employee ID 2031 and PIN 1234.';
  }
  ...
  loginCustomer(): void {
    if (!this.customerEmail.includes('@')) {
      this.accountMessage = 'Enter a valid email address for this simulated account.';
      return;
    }
  ...
  submitSocialFind(): void {
    if (!this.shareFindCaption.trim()) {
      this.socialThankYou = 'Add a short caption before sharing your demo find.';
      return;
    }
    this.socialThankYou = 'Your find was added to the simulated #DollarTree community wall!';
    setTimeout(() => {
  ...
  updatePreference(preference: string): void {
    ...
    if (this.customerLoggedIn) {
      localStorage.setItem(
        'dt2031-customer-profile',
        JSON.stringify({ name: this.customerName, email: this.customerEmail, preference }),
      );
    }
  }
  ...
  exploreDepartment(category: string, aisle?: number): void {
    ...
    const first = this.data.productsForStore(this.selectedStoreId)
      .find((product) => product.category === category && (!aisle || product.aisle === aisle));
  ...
  exploreAisle(aisle: number): void {
    ...
    this.currentProduct = this.data.productsForStore(this.selectedStoreId)
  ...
  missionStopNumber(aisle: number): number {
    return this.missionProducts.findIndex((product) => product.aisle === aisle) + 1;
  }
  ...
  addProduct(product: FutureProduct): void {
    if (this.data.addToCart(product.productId)) {
      ...
      this.messages.push({
        ...
        detail: product.backroomQuantity > 0 ? `There are ${product.backroomQuantity} in the backroom. I can help request a restock.` : `Expected restock: ${product.expectedRestock}.`,
      });
    }
    this.chatOpen = true;
  }
  ...
  nextDemoStep(): void {
    ...
    const actions: Record<number, () => void> = {
      1: () => {
        const balloons = this.data.products().find((product) => product.productId === 'DT1010');
        if (balloons) this.showOnMap(balloons);
      },
      2: () => this.sendPrompt('How many birthday balloons are in stock?'),
      3: () => this.setView('employee'),
      4: () => {
        if (this.employeeLoggedIn) this.sendTeamPrompt('What should I stock next?');
      },
      8: () => this.setView('employee'),
      9: () => document.querySelector('.solink-intelligence')?.scrollIntoView({ behavior: 'smooth' }),
    };
  ...
  startSimilarSearch(): void {
    ...
    this.messages.push({
      ...
      detail: 'Type a product below—try “paper towels,” “toothpaste,” “party cups,” or “storage bins.” I’ll compare similar in-stock options.',
    });
  }
  ...
  findSimilarProducts(): void {
    ...
    const source = this.findProduct(query) ??
      storeProducts.find((product) => product.category.toLowerCase().includes(query));
    let matches: FutureProduct[];
    if (source) {
      this.currentProduct = source;
      matches = storeProducts
        .filter((product) => product.productId !== source.productId && product.category === source.category && product.totalInventory > 0)
        .sort((a, b) => a.price - b.price)
        .slice(0, 4);
    } else {
      matches = storeProducts
        .filter((product) => `${product.name} ${product.category}`.toLowerCase().includes(query) && product.totalInventory > 0)
        .slice(0, 4);
    }
    this.messages.push({ author: 'customer', text: `Find alternatives for ${this.similarSearchText}` });
    this.messages.push({
      author: 'bucky',
      text: matches.length
  ...
  showCategoryInChat(category: string): void {
    const matches = this.data.productsForStore(this.selectedStoreId)
      .filter((product) => product.category === category && product.totalInventory > 0)
      .sort((a, b) => a.price - b.price)
      .slice(0, 4);
    this.messages.push({
      author: 'bucky',
      text: `Here are ${matches.length} ${category.toLowerCase()} picks available at ${this.selectedStore.city}.`,
      detail: 'Tap any item to add it, ask about stock, or open its aisle on the store map.',
      products: matches,
    });
  }
  ...
  toggleVoice(): void {
    ...
    if (!Recognition) {
      this.messages.push({ author: 'bucky', text: 'Voice input is not available in this browser.', detail: 'You can still type your question below.' });
      return;
    }
  ...
  readLastResponse(): void {
    const response = [...this.messages].reverse().find((message) => message.author === 'bucky');
    if (response && 'speechSynthesis' in window) {
      window.speechSynthesis.cancel();
      window.speechSynthesis.speak(new SpeechSynthesisUtterance(`${response.text} ${response.detail ?? ''}`));
    }
  }

  async enableArrivalAlerts(): Promise<void> {
    if (!('Notification' in window)) {
      this.arrivalAlertMessage = 'Arrival alerts are not supported in this browser.';
      return;
    }
    const permission = await Notification.requestPermission();
    this.arrivalAlertsEnabled = permission === 'granted';
    localStorage.setItem('dt2031-arrival-alerts', String(this.arrivalAlertsEnabled));
    this.arrivalAlertMessage = this.arrivalAlertsEnabled
      ? 'Store-arrival alerts are on for this demo.'
  ...
  simulateStoreArrival(): void {
    ...
    new Notification('Welcome to Dollar Tree Future Store #2031', {
      body: 'Bucky is ready with your list, Dollar Zone finds, and the fastest route through the store.',
      icon: 'favicon.svg',
    });
    this.arrivalAlertMessage = 'Demo arrival detected. A welcome notification was sent.';
  }
  ...
  private sendPrompt(prompt: string): void {
    ...
    const followUpProduct =
      this.currentProduct &&
      ['how many', 'where is it', 'in the back', 'restock', 'show me', 'what aisle', 'how much'].some((phrase) => normalized.includes(phrase))
        ? this.currentProduct
        : undefined;
    const product = this.findProduct(normalized) ?? followUpProduct;
    const category = this.findCategory(normalized);
    const budgetMatch = normalized.match(/(?:under|less than|budget(?: of)?)\s*\$?(\d+(?:\.\d{1,2})?)/);
    if (!this.isDollarTreeCustomerQuestion(normalized)) {
      response = {
        author: 'bucky',
        text: 'I can only help with Dollar Tree shopping and store questions.',
        detail: 'Ask me about products, prices, inventory, aisles, your cart, store services, returns, or shopping plans.',
      };
    } else if (normalized.includes('similar') || normalized.includes('alternative')) {
      if (this.currentProduct) {
        const alternatives = this.data.productsForStore(this.selectedStoreId)
          .filter((item) => item.category === this.currentProduct?.category && item.productId !== this.currentProduct?.productId && item.totalInventory > 0)
          .sort((a, b) => a.price - b.price)
          .slice(0, 4);
        response = {
          author: 'bucky',
          text: alternatives.length ? `Here are alternatives to ${this.currentProduct.name}.` : `I don’t see a close in-stock alternative to ${this.currentProduct.name}.`,
          detail: alternatives.length ? 'These are in the same category and sorted by price.' : 'Open Similar Products to search for something else.',
    ...
    } else if (budgetMatch && !(normalized.includes('party') || normalized.includes('birthday'))) {
      const budget = Number(budgetMatch[1]);
      const candidates = this.data.productsForStore(this.selectedStoreId)
        .filter((item) => item.price <= budget && item.totalInventory > 0 && (!category || item.category === category))
        .sort((a, b) => a.price - b.price)
        .slice(0, 4);
      response = {
        author: 'bucky',
      ...
      response.detail = `${response.detail ?? ''} ${pairings.length ? `You may also want ${pairings.map((item) => item.name).join(', ')}.` : ''}`.trim();
      response.products = [product, ...pairings];
    } else if (category) {
      const matches = this.data.productsForStore(this.selectedStoreId)
      ...
      response = {
        ...
        detail: 'Choose one below to check stock, add it to your cart, or see it on the map.',
        products: matches,
      };
      ...
      response = {
        ...
        detail: 'A fictional store-of-the-future concept located in Chesapeake, Virginia.',
      };
    } else if (normalized.includes('$1') || normalized.includes('dollar zone')) {
      const dollarItems = this.data.productsForStore(this.selectedStoreId).filter((item) => item.dollarZone);
      response = {
        author: 'bucky',
        text: `The Dollar Zone has ${dollarItems.length} selected $1 products at ${this.selectedStore.city}.`,
        detail: 'The Dollar Zone is in Aisle 1 near the entrance. Not every store item costs $1.',
        type: 'deals',
        items: dollarItems.slice(0, 7).map((item) => item.name),
      };
    } else if (normalized.includes('arriving today')) {
      const arriving = this.data.productsForStore(this.selectedStoreId).filter((item) => item.inventoryStatus === 'Arriving Today');
      response = {
        author: 'bucky',
        text: `${arriving.length} product is marked as arriving today.`,
        detail: arriving.map((item) => `${item.name}: ${item.expectedRestock}`).join(' • ') || 'No arrivals are currently scheduled.',
      };
    } else if (normalized.includes('under $3') || normalized.includes('under 3')) {
      const snacks = this.data.productsForStore(this.selectedStoreId).filter((item) => item.category === 'Snacks' && item.price < 3);
      response = {
        author: 'bucky',
        text: `I found ${snacks.length} snacks under $3.`,
        detail: snacks.map((item) => `${item.name} — $${item.price.toFixed(2)}`).join(' • '),
        type: 'list',
        items: snacks.map((item) => item.name),
      };
    } else if (normalized.includes('cheaper') || normalized.includes('alternative')) {
      const valuePick = this.data.productsForStore(this.selectedStoreId)
    ...
    } else if ((normalized.includes('party') || normalized.includes('birthday')) && normalized.includes('under $20')) {
      response = {
        author: 'bucky',
        text: 'I built a birthday party bundle for $18.75.',
        detail: 'This value-first plan covers the essentials for about eight guests.',
        type: 'list',
        items: ['Birthday balloons', 'Party cups', 'Candles', 'Napkins', 'Chips', 'Juice boxes'],
      };
    } else if (normalized.includes('batter')) {
      response = {
        author: 'bucky',
        text: 'Batteries are in Aisle 8 near Electronics.',
        detail: 'Solink sees 21 on the shelf and 53 in the backroom. Estimated walk: 42 seconds.',
        type: 'map',
        items: ['Enter through the front', 'Pass Aisle 4', 'Turn right at Aisle 8'],
      };
    } else if (normalized.includes('balloon')) {
      response = {
        author: 'bucky',
        text: 'Birthday balloons are in Party Supplies, Aisle 12.',
        detail: 'Nearby: gift bags, tissue paper, birthday candles, and wrapping paper.',
        type: 'map',
      };
      ...
      response = {
        ...
        type: 'list',
        items: ['Taco shells', 'Salsa', 'Cheese', 'Taco seasoning', 'Paper plates', 'Napkins'],
      };
    } else if (normalized.includes('halloween')) {
      response = {
        author: 'bucky',
        text: 'I built a Halloween decorating bundle for $18.50.',
        detail: 'A festive mix with decorations, candy, pumpkins, LED lights, treat bags, and window clings.',
        type: 'list',
        items: ['Decorations', 'Candy', 'Mini pumpkins', 'LED lights', 'Treat bags', 'Window clings'],
      };
    } else if (normalized.includes('party') || normalized.includes('birthday')) {
      response = {
        author: 'bucky',
        text: 'Your complete birthday list for 12 guests is ready — estimated total $34.80.',
        detail: 'I included decorations, tableware, snacks, drinks, candles, and gift bags.',
        type: 'list',
        items: ['Decorations', 'Plates & napkins', 'Snacks & drinks', 'Candles', 'Gift bags'],
      };
    } else if (normalized.includes('baby shower')) {
      response = {
        author: 'bucky',
        text: 'Your baby shower list for 20 guests is ready — estimated total $42.25.',
        detail: 'This list covers the table, food, games, and favors.',
        type: 'list',
        items: ['Decorations', 'Plates', 'Napkins', 'Tablecloths', 'Drinks', 'Snacks', 'Games', 'Gift bags'],
      };
    } else if (normalized.includes('coke')) {
      response = {
        author: 'bucky',
        text: 'Yes — Solink Vision AI detects 18 Coke products on the shelf and 96 in the backroom.',
        detail: 'An associate is stocking in 12 minutes. Next shipment: tomorrow. Inventory confidence: 99%.',
        type: 'inventory',
      };
      ...
      response = {
        author: 'bucky',
        text: 'I built a classroom essentials bundle for $19.25.',
        detail: 'These are practical, high-utility supplies with strong current availability.',
        type: 'list',
        items: ['Folders', 'Markers', 'Glue', 'Notebooks', 'Pencils', 'Storage bins', 'Whiteboard supplies'],
      };
    } else if (normalized.includes('surprise')) {
      response = {
        author: 'bucky',
        text: "Today's hidden Treasure Hunt finds are ready.",
        detail: 'Unexpected bargains selected from high-rated, in-stock discoveries.',
        type: 'deals',
        items: ['Glass vase', 'LED pumpkin', 'Party cups', 'Reading glasses', 'Cleaning brushes', 'Candles', 'Storage basket'],
      };
      ...
      response = {
        ...
        detail: 'An associate is stocking now. Estimated shelf availability: 15 minutes.',
        type: 'inventory',
      };
    } else if (normalized.includes('truck') || normalized.includes('delivery')) {
      response = {
        author: 'bucky',
        text: 'Delivery operations are restricted to authorized employees.',
        detail: 'Associates, store managers, and district managers can review truck status after signing into the Employee Portal Demo.',
      };
      ...
      response = {
        ...
        type: 'deals',
        items: ['Greeting cards', 'Party cups', 'Notebooks', 'Kitchen towels', 'Select candy'],
      };
    } else if (normalized.includes('order')) {
      response = {
        author: 'bucky',
        text: 'Order DT-3184 has been processed.',
        detail: 'This is simulated order information for the 2031 concept demo.',
      };
      ...
      response = {
        ...
        detail: 'Return eligibility depends on the item condition and store policy.',
      };
    } else if (normalized.includes('paper towel') || normalized.includes('map')) {
      response = {
        author: 'bucky',
      ...
      response = {
        ...
        detail: 'Try asking about an aisle, product stock, a party, today’s truck, or Dollar Deals.',
      };
    }
  ...
  updateTask(task: EmployeeTask, status: string): void {
    ...
    if (status === 'In Progress') {
      ...
      this.taskEscalationMessage = 'Bucky opened help for this task. Try the guided steps or notify a leader.';
  ...
  showTaskHelp(task: EmployeeTask): void {
    ...
    this.activeTaskAssistant = task;
    this.taskAssistantMode = 'help';
    this.taskEscalationMessage = 'Tell Bucky what is blocking you, follow the instructions below, or notify a leader.';
  }
  ...
  notifyTaskLeader(role: 'SM' | 'DM'): void {
    ...
    this.teamMessages.push({ author: 'bucky', text: this.taskEscalationMessage });
  }
  ...
  scanEmployeeItem(simulatedProductId?: string): void {
    const value = (simulatedProductId ?? this.employeeScanText).trim().toLowerCase();
    const product = this.data.products().find((item) =>
      item.productId.toLowerCase() === value || item.name.toLowerCase().includes(value),
    );
    if (!product) {
      this.scannedEmployeeProduct = null;
      this.employeeScanMessage = 'Item not found. Try DT1001, DT1016, or a product name.';
      return;
    }
  ...
  get scannedSalesInsight(): { unitsToday: number; lastSale: string; zeroSalesHours: number; expectedDaily: number } | null {
    if (!this.scannedEmployeeProduct) return null;
    return this.zeroSalesByProduct[this.scannedEmployeeProduct.productId] ?? {
      unitsToday: 2,
  ...
  sendTeamPrompt(prompt: string): void {
    ...
    const paper = this.data.products().find((product) => product.productId === 'DT1001')!;
    const employeeProduct = this.findProduct(query);
    let text = '';
    this.incidentGuide = null;
    this.trainingVideoOpen = false;
    const robberyQuestion = query.includes('robbed') || query.includes('robbery') || query.includes('armed person') || query.includes('hold up');
    const supportRoute = this.findSscSupportRoute(query);
    if (!this.isDollarTreeEmployeeQuestion(query)) {
      this.activeSscRoute = null;
      text = 'I can only help with Dollar Tree work, store procedures, safety, tasks, inventory, customers, schedules, and SSC support. Please ask a work-related question.';
      ...
      text = 'Your safety and everyone else’s safety come first. Do not confront or chase the person. When it is safe, call 911, contact your Store Manager, and connect with Asset Protection. I opened the safety steps and training video below.';
    ...
    } else if (query.includes('my schedule') || query.includes('work this week') || query.includes('shift this week')) {
      const upcoming = this.weeklySchedule.filter((shift) => shift.status === 'Today' || shift.status === 'Scheduled');
    ...
    } else if ((query.includes('zero sales') || query.includes('on hand') || query.includes('scan')) && this.scannedEmployeeProduct) {
      ...
      text = `${this.lowStockProducts.length} products need attention. Start with ${this.lowStockProducts.slice(0, 3).map((item) => `${item.name} in Aisle ${item.aisle}`).join(', ')}.`;
      ...
      const overdue = this.data.tasks().filter((task) => task.status !== 'Complete' && task.dueTime.includes('9:'));
      text = overdue.length ? `${overdue.length} task is overdue: ${overdue.map((task) => task.name).join(', ')}.` : 'No tasks are currently overdue.';
    } else if (query.includes('damaged')) {
      text = 'Move the item to the designated damage bin, scan its product ID, select the damage reason, attach a photo in the real workflow, and notify a manager if the item is hazardous.';
    } else if (query.includes('accuracy') || query.includes('mismatch')) {
      text = 'Three shelf-count mismatches need verification: toothpaste in Aisle 8, cleaning spray in Aisle 6, and storage trays in Aisle 9.';
    } else if (query.includes('customer')) {
      text = 'Ask for the product name, then check shelf and backroom quantities for Future Store #2031. For unavailable items, offer a similar product or a simulated restock alert.';
    } else if (query.includes('how do i stock seasonal') || query.includes('stock seasonal')) {
      text = 'Step 1: Bring the stocking cart. Step 2: Check the shelf label. Step 3: Put older items in front. Step 4: Fill empty spaces. Step 5: Press Complete.';
  ...
  private isDollarTreeCustomerQuestion(query: string): boolean {
    const terms = ['dollar tree', 'product', 'item', 'find', 'where', 'aisle', 'stock', 'shelf', 'backroom', 'price', 'cost', 'deal', '$1', 'dollar zone', 'cart', 'party', 'birthday', 'baby shower', 'taco', 'halloween', 'snack', 'drink', 'clean', 'battery', 'balloon', 'storage', 'teacher', 'store', 'hour', 'close', 'return', 'ebt', 'order', 'pickup', 'map', 'shop', 'alternative', 'similar', 'under $'];
    return terms.some((term) => query.includes(term)) || Boolean(this.findProduct(query)) || Boolean(this.findCategory(query));
  }

  private isDollarTreeEmployeeQuestion(query: string): boolean {
    const terms = ['dollar tree', 'store', 'task', 'priority', 'stock', 'aisle', 'shelf', 'backroom', 'inventory', 'on hand', 'zero sales', 'scan', 'item id', 'truck', 'pallet', 'customer', 'damage', 'schedule', 'shift', 'manager', 'associate', 'ssc', 'support', 'asset', 'shrink', 'theft', 'rob', 'armed', 'safety', 'injury', 'emergency', 'gold', 'register', 'pos', 'zebra', 'technology', 'payroll', 'benefit', 'hr', 'shipment', 'vendor', 'delivery', 'planogram', 'policy'];
    return terms.some((term) => query.includes(term)) || Boolean(this.findProduct(query)) || Boolean(this.findCategory(query));
  }

  private findSscSupportRoute(query: string): SscSupportRoute | null {
    if (query.includes('asset protection') || query.includes('asset protect') || query.includes('shrink') || query.includes('theft') || query.includes('robbed') || query.includes('robbery') || query.includes('armed person') || query.includes('hold up') || query.includes('register exception')) {
      return {
        team: 'SSC Asset Protection Operations',
        specialist: 'Asset Protection Response Specialist',
        channel: 'the secure AP support queue',
        priority: query.includes('urgent') || query.includes('safety') ? 'Urgent' : 'Priority',
        gather: ['store number', 'time of the event', 'location in the store', 'incident type'],
      };
    }
    if (query.includes('gold score') || query.includes('gold scores') || query.includes('gold metric') || query.includes('store score')) {
      return {
        team: 'SSC GOLD Performance Support',
        specialist: 'Retail Performance Partner',
        channel: 'the operations support queue',
        priority: 'Standard',
        gather: ['store number', 'scorecard period', 'metric needing review', 'a screenshot if available'],
      };
    }
    if (query.includes('injury') || query.includes('unsafe') || query.includes('safety') || query.includes('emergency')) {
      return {
        team: 'SSC Safety and Risk Support',
        specialist: 'Safety Response Specialist',
        channel: 'the priority safety line',
        priority: 'Urgent',
        gather: ['store number', 'exact location', 'what happened', 'whether anyone needs immediate help'],
      };
    }
    if (query.includes('register') || query.includes('pos') || query.includes('computer') || query.includes('zebra') || query.includes('technology') || query.includes('system down')) {
      return {
        ...
        gather: ['store number', 'device or register number', 'error message', 'steps already tried'],
      };
    }
    if (query.includes('payroll') || query.includes('schedule') || query.includes('benefit') || query.includes('hr') || query.includes('people support')) {
      return {
    ...
    if (query.includes('inventory') || query.includes('shipment') || query.includes('invoice') || query.includes('vendor') || query.includes('delivery issue')) {
      return {
        team: 'SSC Inventory and Supply Chain Support',
        specialist: 'Store Inventory Support Analyst',
        channel: 'the inventory support queue',
        priority: 'Standard',
        gather: ['store number', 'item or shipment number', 'quantity expected', 'quantity received'],
      };
    }
    if (query.includes('ssc') || query.includes('support center') || query.includes('issue in the store') || query.includes('store issue') || query.includes('connect me') || query.includes('right person')) {
      return {
        team: 'SSC Store Operations Support',
        specialist: 'Store Operations Duty Specialist',
        channel: 'the store-support triage queue',
        priority: 'Standard',
        gather: ['store number', 'short issue summary', 'business impact', 'help needed'],
      };
    }
    return null;
  }
  ...
  private findProduct(query: string): FutureProduct | undefined {
    ...
    return products.find((product) => {
      ...
      return query.includes(product.name.toLowerCase()) || terms.some((term) => query.includes(term));
    });
  }

  private getProductPairings(product: FutureProduct): FutureProduct[] {
    const pairingTerms: Record<string, string[]> = {
      'Party Supplies': ['candle', 'napkin', 'cup', 'balloon', 'gift'],
      Cleaning: ['sponge', 'towel', 'spray', 'trash', 'microfiber'],
      Snacks: ['water', 'soda', 'cup', 'plate', 'candy'],
      Drinks: ['snack', 'cup', 'plate', 'candy'],
      'School Supplies': ['pencil', 'notebook', 'folder', 'glue', 'marker'],
      'Health & Beauty': ['toothbrush', 'soap', 'tissue', 'storage'],
      'Home Organization': ['basket', 'container', 'label', 'towel'],
      Seasonal: ['light', 'candy', 'decoration', 'treat'],
      'The Dollar Zone': ['card', 'cup', 'notebook', 'candy'],
    };
    const terms = pairingTerms[product.category] ?? [];
    const available = this.data.productsForStore(this.selectedStoreId)
      .filter((item) => item.productId !== product.productId && item.totalInventory > 0);
    const preferred = available.filter((item) =>
      terms.some((term) => `${item.name} ${item.category}`.toLowerCase().includes(term)),
    );
    const fallback = available.filter((item) => item.category === product.category);
  ...
  private productResponse(product: FutureProduct, query: string): ChatMessage {
    ...
    if (query.includes('where') || query.includes('find') || query.includes('map')) {
      return {
        author: 'bucky',
        text: location,
    ...
    if (product.totalInventory === 0) {
      const alternative = this.data.productsForStore(this.selectedStoreId).find((item) => item.category === product.category && item.productId !== product.productId && item.totalInventory > 0);
      return {
        author: 'bucky',
